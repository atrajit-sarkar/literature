...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typingText) {
        const phrases = [
            'Poems & Small Prose ✒️',
            'Minimal words. Lasting echoes ✨',
            'Quiet pages for careful reading',
            'Collected pieces — simply arranged',
            'A portfolio of verses �'
        ];
    ...
    // Optional: copy email on click (keeps default mailto behavior if modifier keys not used)
    const emailLinks = document.querySelectorAll('a[href^="mailto:"]');
    emailLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            if (e.ctrlKey || e.metaKey || e.shiftKey) return; // allow default when modifiers
            // copy but also open mail client
